...
static int do_test(const char *description, int value, int expected, int negate,
		   int fatal)
{
	int num_chars;
...
static void test_title(const char *title)
{
...
static int test1(void)
{
	...
	err |= test("major",
		    system("cat /proc/devices | grep '" XSTR(
			    COM1_MAJOR) " " MODULE_NAME "' >/dev/null 2>&1"),
		    0);
	err |= test("ioports COM1",
		    system("cat /proc/ioports | grep '03f8-03ff : " MODULE_NAME
			   "' > /dev/null 2>&1"),
		    0);
	err |= test("ioports COM2",
		    system("cat /proc/ioports | grep '02f8-02ff : " MODULE_NAME
			   "' > /dev/null 2>&1"),
		    0);
	err |= test("interrupts COM1",
		    system("cat /proc/interrupts | grep '4:.*" MODULE_NAME
			   "' > /dev/null 2>&1"),
		    0);
	err |= test("interrupts COM2",
		    system("cat /proc/interrupts | grep '3:.*" MODULE_NAME
			   "' > /dev/null 2>&1"),
		    0);
	...
	err |= fatal_test(
		"insmod " MODULE_NAME ", major=" XSTR(COM2_MAJOR),
		system("insmod " MODULE_NAME ".ko major=" XSTR(COM2_MAJOR)), 0);
	err |= test("major",
		    system("cat /proc/devices | grep '" XSTR(
			    COM2_MAJOR) " " MODULE_NAME "' >/dev/null 2>&1"),
		    0);
	err |= test("rmmod", system("rmmod " MODULE_NAME), 0);
	if (err)
		return err;
	...
	err |= fatal_test("insmod " MODULE_NAME ", COM2 only",
			  system("insmod " MODULE_NAME
				 ".ko option=" XSTR(OPTION_COM2_ONLY)),
			  0);
	err |= fatal_test("insmod " SOLUTION_NAME ", COM1 only",
			  system("insmod " SOLUTION_NAME
				 ".ko option=" XSTR(OPTION_COM1_ONLY)),
			  0);
	err |= test(
		"ioports COM1",
		system("cat /proc/ioports | grep '03f8-03ff : " SOLUTION_NAME
		       "' > /dev/null 2>&1"),
		0);
	err |= test("ioports COM2",
		    system("cat /proc/ioports | grep '02f8-02ff : " MODULE_NAME
		    ...
		    0);
	err |= test("interrupts COM1",
		    system("cat /proc/interrupts | grep '4:.*" SOLUTION_NAME
			   "' > /dev/null 2>&1"),
		    0);
	err |= test("interrupts COM2",
		    system("cat /proc/interrupts | grep '3:.*" MODULE_NAME
...
static const struct {
	...
	{
		.num = 3,
		.speed = { UART16550_BAUD_1200, UART16550_BAUD_2400,
			   UART16550_BAUD_4800, -1 },
		.bufsizes = { 128, 256 },
	},
	{
		.num = 4,
		.speed = { UART16550_BAUD_9600, UART16550_BAUD_19200,
			   UART16550_BAUD_38400, UART16550_BAUD_56000 },
		.bufsizes = { 256, 1024 },
	},
	{
		.num = 1,
...
static void gen_params(struct uart16550_line_info *line, int speed_set)
{
	int r;

	line->baud =
		speed_sets[speed_set].speed[rand() % speed_sets[speed_set].num];
...
static int gen_test_file(char *fname, int speed_set)
{
	int size, min, max;
	char comm[1024];
...
static void copy_file(int fdr, int fdw, int len)
{
...
static int copy_test(int fd0, int fd1, int speed_set)
{
	pid_t rpid, wpid;
...
static int generic_test(const char *reader, const char *writer, int speed_set,
			int num_tests)
{
	int fd0, fd1, i, err = 0;
	...
	if (reader != writer) {
		...
		sprintf(cbuf, "insmod %s.ko option=%d", reader,
			OPTION_COM2_ONLY);
		fatal_test(dbuf, system(cbuf), 0);
		sprintf(dbuf, "insmod %s", writer);
		sprintf(cbuf, "insmod %s.ko option=%d", writer,
			OPTION_COM1_ONLY);
		fatal_test(dbuf, system(cbuf), 0);
	} else {
		sprintf(dbuf, "insmod %s", reader);
	...
	if (fd1 == -1)
		fail("open " UART1);
	err |= test("ioctl reader", ioctl(fd1, UART16550_IOCTL_SET_LINE, &uli),
		    0);
	err |= test("ioctl writer", ioctl(fd0, UART16550_IOCTL_SET_LINE, &uli),
		    0);
